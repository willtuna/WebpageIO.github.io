...
def data_generator(price,id,date):
    ...
    if(maincat == 0):
        ...
        print(in_out[maincat]+','+income[secondcat]+',無,'+str(price)+','+str(id)+','+date)
    else:
        secondcat = random.randint(0,6)
        if( secondcat == 0):
            thirdcat = random.randint(0,2)
            print(in_out[maincat]+','+outcome[secondcat]+','+eat[thirdcat]+','+str(price)+','+str(id)+','+date)
        elif (secondcat == 1):
            thirdcat = random.randint(0,4)
            print(in_out[maincat]+','+outcome[secondcat]+','+transport[thirdcat]+','+str(price)+','+str(id)+','+date)
        else:
